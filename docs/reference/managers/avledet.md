...
### `Avledet.world_time_multiplier`
  ...
  (might have weird client time-skip effects). Setting to a values greater than 1 will speed up the in-game time.
  
  > A simple use-case of this is the in-game sleep time-skip, which rapidly progresses the time until morning. See the 
...
> Returns `number` | **readonly**
  ...
  > The worldTime for the nighttime pertaining to the current `day` + 1
...
> Most of the events above are prefix methods like in Harmony. Similarly, there is a `POST` event that can be 
  ...
  > Handlers using `POST` cannot be canceled unless otherwise stated (because what would be the point?).